...
public class Asteroid extends VectorSprite {
	...
	public Asteroid(double x, double y, double xspeed, double yspeed,
			int sizemod) {
		...
		for (int i = 0; i < 360; i += (360 / 6)) {
			shape.addPoint((int) (40 / sizemod * Math.cos(Math.toRadians(i))),
	...
	public void die(Laser rlaser) {
		...
		if (this.sizemod < 3) {
			...
			for (int j = 0; j < (r.nextInt(4) + 2); j++) {
				...
				new MirrorAsteroid(this.xpos, this.ypos, r.nextFloat() * speed
						- speed / 2, r.nextFloat() * speed - speed / 2,
						this.sizemod + 1);
				}else{
				new Asteroid(this.xpos, this.ypos, r.nextFloat() * speed
						- speed / 2, r.nextFloat() * speed - speed / 2,
						this.sizemod + 1);
				}
			}
