...
public class Ship extends VectorSprite {
	...
	public void update() {
		...
		for (int i = 0; thrust.npoints != 0 && i < thrust.npoints; i++) {
			int x = (int) Math.round((Math.cos(angle - 3 * Math.PI / 2)
					* thrust.xpoints[i] - Math.sin(angle - 3 * Math.PI / 2)
					* thrust.ypoints[i]));
			int y = (int) Math.round((Math.sin(angle - 3 * Math.PI / 2)
					* thrust.xpoints[i] + Math.cos(angle - 3 * Math.PI / 2)
					* thrust.ypoints[i]));
	...
	public void shoot() {
		if (this.alive == true) {
			if (count >= ShootDelay) {
				Asteroids.lasers
						.add(new Laser(this.xpos, this.ypos, this.angle,Color.RED));
