...
public abstract class VectorSprite {
	...
	public boolean colide(VectorSprite s) {
		for (int i = 0; i < s.drawshape.npoints; i++) {
			if (this.drawshape.contains(s.drawshape.xpoints[i],
					s.drawshape.ypoints[i])) {
				return true;
			}
		}
		return false;
	...
	public void update() {
		...
		for (int i = 0; shape.npoints != 0 && i < shape.npoints; i++) {
			x = (int) Math.round((Math.cos(angle) * shape.xpoints[i] - Math
					.sin(angle) * shape.ypoints[i]));
			y = (int) Math.round((Math.sin(angle) * shape.xpoints[i] + Math
					.cos(angle) * shape.ypoints[i]));
